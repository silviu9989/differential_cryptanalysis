...
int main () {
    ...
    FILE* fisier1;
    fisier1 = fopen("pairs-k2_delta_in_bbbb.txt", "r");
    fseek(fisier1, 0, 0);

    int i=0; 
    int a=0, b=0;
    int k2_delta_in_bbbb[100][2];

    fscanf (fisier1, "%x", &i);
    k2_delta_in_bbbb[0][0] = i;
    ...
    while(k<=0xffff)
    {
        for(a=0; a < 100; a++)
        {
            T=k2_delta_in_bbbb[a][0] ^ k5;
            Tprime=k2_delta_in_bbbb[a][1] ^ k5;
    ...
    for(i=0; i<65536;i++)
    {
            //printf("%i ", mat_prob[i]); //PRINT LE MATRIX OF POINTS     
            if(max < mat_prob[i])
            {
                row=i;
                max=mat_prob[i];
    ...
    printf("After the first step, the guess is that k2=%x\n", row);
}